...
<article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
    ...
    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
        ...
        @if($post->category == true) {{-- False est equivalent a null --}}
        ...
        {{-- isNotEmpty() fonctionne pour voir si une collection n'est pas vide --}}
        ...
        <p class="text-xl lg:text-2xl text-slate-600">
            ...
            @if($list == true)
                {{ $post->excerpt }}  
            @else
                {!! nl2br(e($post->content)) !!} {{-- Affiche la description complete avec espaces  --}}
            @endif
        </p>
        ...
        {{-- Directive Blade situé dans Providers/AppServiceProvider.php method boot() ---- @datetime pour convertir en format date et heure souhaité --}}
        @endif
    </div>
</article>
