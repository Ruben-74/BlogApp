<x-default-layout :title="$post->title">
    ...
    <div class="space-y-10 md:space-y-16">
        ...
        <x-post :post="$post" /> {{-- On enleve l'attribut list pour laisser la valeur false par defaut --}}
        ...
        <form action="{{ route('posts.comment', ['post' => $post]) }}" method="POST">
            ...
            <div class="flex h-12">
                <input class="w-full bg-slate-50 rounded-lg px-5 text-slate-900 focus:outline focus:outline-2 focus:outline-indigo-500"
                    type="text" name="comment" placeholder="Quelque chose à rajouter ? 🎉" autocomplete="off">
                <button
            ...
            @error('comment')
                <p class="mt-2 txt-sm text-red-600">{{ $message }}</p>
            @enderror
        </form>
        @endauth
